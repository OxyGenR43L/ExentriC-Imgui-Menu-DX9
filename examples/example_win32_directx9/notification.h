...
#define NOTIFY_PADDING_MESSAGE_Y		5.f		
#define NOTIFY_FADE_IN_OUT_TIME			300
...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(NOTIFY_PADDING_X, NOTIFY_PADDING_Y));
			...
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);
			{
				...
				TextColored(text_color, addition::cheat_name);
				PopFont();
